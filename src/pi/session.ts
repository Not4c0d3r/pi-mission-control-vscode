...
export class PiSessionManager {
    ...
    private _registerMissionTools(session: AgentSession): void {
        const controller = missionManager.controller;
        if (!controller) return;

        session.registerTool({
            name: 'mc_start_mission',
            description: 'Start a new mission with a goal and a list of tasks.',
            input: { goal: { type: 'string' }, tasks: { type: 'array', items: { type: 'string' } } },
            handler: async ({ goal, tasks }) => await controller.startMission(goal, tasks)
        });

        session.registerTool({
            name: 'mc_expand_to_blueprint',
            description: 'Expand a specific task into a detailed blueprint.',
            input: { taskId: { type: 'string' }, details: { type: 'string' } },
            handler: async ({ taskId, details }) => await controller.expandTask(taskId, details)
        });

        session.registerTool({
            name: 'mc_approve_blueprint',
            description: 'Approve the current blueprint and move the mission to the EXECUTING phase.',
            handler: async () => await controller.approveBlueprint()
        });

        session.registerTool({
            name: 'mc_checkpoint',
            description: 'Mark a task as completed or blocked.',
            input: { taskId: { type: 'string' }, status: { type: 'string', enum: ['completed', 'blocked'] }, notes: { type: 'string' } },
            handler: async ({ taskId, status }) => await controller.checkpointTask(taskId, status)
        });

        session.registerTool({
            name: 'mc_pivot_plan',
            description: 'Pivot the mission plan based on new information.',
            input: { update: { type: 'string' }, taskId: { type: 'string' }, newBlueprint: { type: 'string' } },
            handler: async ({ update, taskId, newBlueprint }) => await controller.pivotPlan(update, taskId, newBlueprint)
        });

        session.registerTool({
            name: 'mc_finalize_mission',
            description: 'Finalize the mission and mark it as DONE.',
            handler: async () => await controller.finalizeMission()
        });
    }
